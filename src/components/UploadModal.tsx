...
const UploadModal = ({ isOpen, onClose, onUpload }: UploadModalProps) => {
  ...
  const getFileIcon = (fileType: string): JSX.Element => {
    ...
    if (fileType.includes('doc') || fileType.includes('word')) return <FileText className="h-4 w-4 text-blue-500" />;
    if (fileType.includes('xls') || fileType.includes('sheet')) return <FileText className="h-4 w-4 text-green-500" />;
    if (fileType.includes('csv')) return <FileText className="h-4 w-4 text-green-500" />;
    if (fileType.includes('ppt') || fileType.includes('presentation')) return <FileText className="h-4 w-4 text-orange-500" />;
    if (fileType.includes('image') || fileType.includes('png') || fileType.includes('jpg')) return <FileText className="h-4 w-4 text-purple-500" />;
                ...
                <div className="mt-6">
                  ...
                  >
                    ...
                    <p className="mt-2 text-sm text-gray-600">
                      Dépose tes fichiers ici ou <span className="font-medium text-blue-600">clique pour choisir</span>
                    </p>
                  </div>
                </div>
